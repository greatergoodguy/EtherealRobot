...
public class EtherealDualCHGUI : MonoBehaviour {
	...
	private void DrawTextureCenter(float posX, float posY, Texture texture){
		GUI.DrawTexture(new Rect(posX/2 - texture.width/2, posY - texture.height/2, texture.width, texture.height), texture);
		GUI.DrawTexture(new Rect((posX/2 + Screen.width / 2) - texture.width/2, posY - texture.height/2, texture.width, texture.height), texture);
	}
}
