Shader "BB_RimLighting"
{
	Properties 
	{
_DiffuseColor("_DiffuseColor", Color) = (1,1,1,1)
_RimPower("_RimPower", Range(0.1,3) ) = 1.707772
_Specular_colar_multiplier("_Specular_colar_multiplier", Color) = (1,1,1,1)
_specular_gloss("specular alpha gloss", 2D) = "gray" {}
_emmisive_map("_emmisive_map", 2D) = "gray" {}
_normal("_normal", 2D) = "gray" {}
_diffuse_alpha("diffuse alpha gloss", 2D) = "gray" {}
...
float _RimPower;
float4 _Specular_colar_multiplier;
sampler2D _specular_gloss;
sampler2D _emmisive_map;
sampler2D _normal;
...
float2 uv_normal;
float2 uv_emmisive_map;
float3 viewDir;
float2 uv_specular_gloss;
...
float4 Tex2D1=tex2D(_diffuse_alpha,(IN.uv_diffuse_alpha.xyxy).xy);
float4 Tex2DNormal0=float4(UnpackNormal( tex2D(_normal,(IN.uv_normal.xyxy).xy)).xyz, 1.0 );
float4 Tex2D2=tex2D(_emmisive_map,(IN.uv_emmisive_map.xyxy).xy);
...
float4 Pow1=pow(Fresnel1,_RimPower.xxxx);
float4 Tex2D0=tex2D(_specular_gloss,(IN.uv_specular_gloss.xyxy).xy);
float4 Multiply2=Pow1 * Tex2D0;
float4 Multiply1=Tex2D0.aaaa * _Specular_colar_multiplier;
