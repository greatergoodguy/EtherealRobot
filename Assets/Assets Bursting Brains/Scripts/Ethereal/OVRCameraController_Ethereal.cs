...
//
public class OVRCameraController_Ethereal : CameraController_BB {
	...
	bool ConfigureCamera(ref Camera camera, float distOffset, float perspOffset, float eyePositionOffset)
	{
		...
		camera.GetComponent<OVRCamera_Ethereal>().SetPerspectiveOffset(ref PerspOffset);
		...
		// accurate
		camera.GetComponent<OVRCamera_Ethereal>().NeckPosition = NeckPosition;
		EyePosition.x = eyePositionOffset; 
			
		camera.GetComponent<OVRCamera_Ethereal>().EyePosition = EyePosition;		
