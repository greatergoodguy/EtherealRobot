...
//
public class OVRCameraController_BB : CameraController_BB {
	...
	bool ConfigureCamera(ref Camera camera, float distOffset, float perspOffset, float eyePositionOffset)
	{
		...
		camera.GetComponent<OVRCamera_BB>().SetPerspectiveOffset(ref PerspOffset);
		...
		// accurate
		camera.GetComponent<OVRCamera_BB>().NeckPosition = NeckPosition;
		EyePosition.x = eyePositionOffset; 
			
		camera.GetComponent<OVRCamera_BB>().EyePosition = EyePosition;		
